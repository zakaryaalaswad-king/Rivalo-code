    ...
    <div className="min-h-[calc(100vh-64px)] flex items-center justify-center px-6 py-16">
      <div className="max-w-lg w-full bg-[#0A0C22] border border-[#3B82F6]/40 p-10 text-center" data-testid="payment-success-page">
        ...
        {(status === "expired" || status === "timeout" || status === "error") && (
          <>
            <div className="font-display text-3xl mt-4">Payment not confirmed</div>
            <p className="text-slate-400 mt-3">{status === "expired" ? "The Stripe session expired." : "We couldn't confirm payment yet. Try refreshing your dashboard in a minute."}</p>
            <button onClick={() => nav("/dashboard")} className="mt-6 border border-white/20 px-6 py-3 hover:bg-white/5">Go to dashboard</button>
          </>
        )}
      </div>
    </div>
