  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/60 backdrop-blur-sm" data-testid="verify-email-modal">
      <div className="card p-8 max-w-md w-full relative reveal">
        <button onClick={onClose} className="absolute top-3 right-3 text-slate-400 hover:text-white p-1" data-testid="verify-modal-close"><X size={18} /></button>
        ...
        <form onSubmit={verify} className="mt-5 space-y-3">
          ...
          <button type="button" onClick={send} disabled={sending} className="w-full text-sm text-slate-400 hover:text-white py-2" data-testid="verify-resend-btn">
