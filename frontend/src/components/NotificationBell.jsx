  ...
  return (
    <div className="relative" ref={ref}>
      <button onClick={() => { setOpen((o) => !o); if (!open) refresh(); }} className="relative text-slate-300 hover:text-white p-2" data-testid="notification-bell-btn" aria-label="Notifications">
        <Bell size={18} />
        {data.unread > 0 && (
          <span className="absolute -top-0.5 -right-0.5 min-w-[18px] h-[18px] px-1 bg-[#D4AF37] text-black text-[10px] font-bold flex items-center justify-center" data-testid="notification-unread-count">
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-80 bg-[#0A0C22] border border-white/10 backdrop-blur-xl shadow-2xl z-50" data-testid="notification-dropdown">
          <div className="flex items-center justify-between px-4 py-3 border-b border-white/5">
            ...
            {data.unread > 0 && (
              <button onClick={markAll} className="text-xs text-[#D4AF37] hover:underline inline-flex items-center gap-1" data-testid="notif-mark-all-read">
          ...
          <div className="max-h-96 overflow-auto">
            ...
            {data.items.map((n) => (
              <Link to={n.link || "#"} key={n.id} onClick={() => setOpen(false)} className={`block px-4 py-3 border-b border-white/5 hover:bg-white/5 ${!n.read ? "bg-[#D4AF37]/5" : ""}`} data-testid={`notif-${n.id}`}>
                <div className="flex items-start gap-2">
                  {!n.read && <span className="mt-1.5 w-2 h-2 rounded-full bg-[#D4AF37] shrink-0" />}
